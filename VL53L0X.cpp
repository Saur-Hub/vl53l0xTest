...
#define ADDRESS_DEFAULT 0b0101001 // default address of the sensor
...
constexpr uint32_t VL53L0XI2CTimeout = 25;
...
VL53L0X::VL53L0X(SharedI2CMaster &dev, bool addressLSB) noexcept :
		SharedI2CClient(dev,
				(addressLSB) ? ADDRESS_DEFAULT | 0x0001 : ADDRESS_DEFAULT)
				, io_timeout(0) // no timeout
				, did_timeout(false)
{
...
bool VL53L0X::CheckPresent() noexcept
{
	if (readReg(IDENTIFICATION_MODEL_ID) == 0xEE)
	{
		return true;
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)  // working fine
{
	/*
	 ...
	 bus->write(value);
	 = bus->endTransmission();*/
	last_status = WriteRegister(reg, value);
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
	 ...
	 */
	last_status = WriteRegister(reg, (value >> 8) & 0xFF);
	last_status = WriteRegister(reg, value & 0xFF);
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
	...
	WriteRegister(reg, (value >> 24) & 0xFF);   // value highest byte
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
	...
	last_status = ReadRegister(reg, value);
	return value;
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
	 ...
	 */
	ReadRegister(reg, val);		// read high byte
	value = (uint16_t) val << 8;		// typecasted to 32 bit

	ReadRegister(reg, val);		// read low byte
	value |= val;
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
	...
	/*
	 ...
	 bus->requestFrom(address, (uint8_t)4);		// should we use read multiple
	...
	ReadRegisters(reg, val, 4);				// how will we evaluate this??
	value = (uint32_t) val[0] << 24;			// aise hi hona chahiye lets see
	value |= (uint32_t) val[1] << 16;
	value |= (uint16_t) val[2] << 8;
	value |= val[3];
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t *src, uint8_t count)
{
	/*bus->beginTransmission(address);
	 ...
	 last_status = bus->endTransmission();
	 */
	last_status = WriteRegisters(reg, src, count);
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t *dst, uint8_t count)
{
	 ...
	 */
	last_status = ReadRegisters(reg, dst, count);
}
...
bool VL53L0X::ReadRegisters(uint8_t reg, uint8_t *buffer,
		size_t numToRead) noexcept	// To read multiple registers from address
{
	/*| 0x80 why added this??*/

	return Transfer((uint8_t) reg, buffer, 1, numToRead, VL53L0XI2CTimeout);// added timeout to 25ms
}

// Read from 8 bit register
bool VL53L0X::ReadRegister(uint8_t reg, uint8_t &val) noexcept// To read from single register
{
	return Transfer((uint8_t) reg, &val, 1, 1, VL53L0XI2CTimeout);
}

bool VL53L0X::WriteRegisters(uint8_t reg, uint8_t *buffer,
		size_t numToWrite) noexcept
{
	return Transfer((uint8_t) reg, buffer, 1 + numToWrite, 0, VL53L0XI2CTimeout);
}

// Write 8 bit register
bool VL53L0X::WriteRegister(uint8_t reg, uint8_t val) noexcept
{
	
	last_status = Transfer((uint8_t) reg, &val, 2, 0, VL53L0XI2CTimeout);
	return last_status;
}
