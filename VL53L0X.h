...
#if SUPPORT_I2C_SENSORS && SUPPORT_VL53L0X

#include "SharedI2CClient.h"	// bro don't use wire.h use sharedI2Client as an Master define everywhere

class VL53L0X: public SharedI2CClient
{
public:
	...
	VL53L0X(SharedI2CMaster &dev, bool addressLSB) noexcept;
...
private:
	...
	bool ReadRegisters(uint8_t reg, uint8_t *buffer, size_t numToRead) noexcept;
	bool WriteRegisters(uint8_t reg, uint8_t *buffer, size_t numToWrite) noexcept;
	bool ReadRegister(uint8_t reg, uint8_t &val) noexcept;
	bool WriteRegister(uint8_t reg, uint8_t val) noexcept;
};
